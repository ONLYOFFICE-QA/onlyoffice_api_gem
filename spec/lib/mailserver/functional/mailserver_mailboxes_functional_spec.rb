...
describe '[Mailserver][Functional] Mailboxes' do
  ...
  let(:domain_id) { api.mailserver.tenant_domain_list.body['response'].first['id'] }
  let(:add_mailbox) do
    api.mailserver.add_mailbox(name: mailbox_name,
